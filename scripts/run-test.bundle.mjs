...
var saveToProfileFn = async ({ session, objectRef, input, expected, interpolate, persistVariable, persistApiConfig }) => {
  ...
  if (trimmed?.startsWith("{")) {
    try {
      const config = JSON.parse(interpolate(expected));
...
// scripts/run-test.mjs
var ROOT = "/Users/coinhat/Documents/jkauto";
var RESET = "\x1B[0m";
var GREEN = "\x1B[32m";
var RED = "\x1B[31m";
var YELLOW = "\x1B[33m";
...
var DIM = "\x1B[2m";
var BOLD = "\x1B[1m";
var [, , tcArg, projectArg, profileName = "default"] = process.argv;
if (!tcArg) {
  console.error("Usage: node scripts/run-test.mjs <testcase.yaml> [project-dir] [profile-name]");
  process.exit(1);
}
var tcPath = path2.resolve(ROOT, tcArg);
var projectPath = projectArg ? path2.resolve(ROOT, projectArg) : path2.dirname(tcPath);
async function loadProfile(projPath, name) {
  const candidates = [
    path2.join(projPath, "profiles", `${name}.env.json`),
    path2.join(projPath, "profiles", `${name}.json`)
  ];
  for (const f of candidates) {
    try {
      const raw = await fs2.readFile(f, "utf-8");
...
async function loadObjectRepositories(projPath) {
  ...
  for (const dir of ["object-repository", "api-request"]) {
    const repoDir = path2.join(projPath, dir);
    try {
      const entries = await fs2.readdir(repoDir);
      for (const entry of entries) {
        if (!entry.endsWith(".objects.json")) continue;
        try {
          const raw = await fs2.readFile(path2.join(repoDir, entry), "utf-8");
...
function normalizeTestCase(tcData) {
  ...
  return {
    ...
    createdAt: tcData.createdAt ?? (/* @__PURE__ */ new Date()).toISOString(),
    updatedAt: tcData.updatedAt ?? (/* @__PURE__ */ new Date()).toISOString()
  };
}
async function readTestCase(filePath) {
  const raw = await fs2.readFile(filePath, "utf-8");
  return normalizeTestCase((0, import_yaml.parse)(raw));
}
async function runSingle(filePath, profile, objectRepositories) {
  const tc = await readTestCase(filePath);
  const runId = randomUUID();
  console.log(`
${BOLD}${CYAN}\u25B6 ${tc.name}${RESET}  ${DIM}(${path2.relative(ROOT, filePath)})${RESET}`);
...
`);
    ...
    void 0,
    { headless: process.env.HEADLESS !== "0", objectRepositories, screenshotDir: "/tmp/jkauto-debug", screenshotMode: "failure", loadTestCase: (p) => readTestCase(path2.resolve(path2.dirname(filePath), p)) }
  );
...
async function main() {
  ...
  const raw = await fs2.readFile(tcPath, "utf-8");
...
`);
    ...
    for (const item of enabledItems) {
      const itemPath = path2.resolve(projectPath, item.testCasePath);
