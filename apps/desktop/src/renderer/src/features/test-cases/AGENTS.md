...
- **Unified http-request body:** `http-request` keyword now carries an optional JSON body via `expected` for POST/PUT/PATCH/DELETE (engine `keywords/defs/api.ts` `httpRequestFn` — adds `Content-Type: application/json` when body present + method allows it). `ApiStepDetail.tsx` `HttpRequestForm` shows the Request Body textarea whenever the method is a body method (not only the legacy `http-request-body` keyword). Engine `resolveUrl()` throws a clear "Base URL not configured" error when a relative path is sent with an empty/unresolved `{{...}}` baseUrl (fixes silent "Failed to parse URL"). Agent guidance updated in `agent/jkauto-mcp.ts` RULES['api-steps'] + `agent/prompt.ts` apply-steps example (body in `expected`, set-base-url first).
- **assert-json-path existence check:** `expected="*"` or empty asserts the field merely exists / is non-empty (engine `assertJsonPathFn`), for dynamic values (accessToken, refreshToken, id, timestamps) the agent can't know exactly. Exact-value asserts still work for known values. Documented in `agent/jkauto-mcp.ts` RULES['api-steps'].
- **set-variable extract mode:** `setVariableFn` now supports extracting from the last response — `expected`=JSON path (e.g. `accessToken`, `data.id`) stores that value into the named variable; `input`=literal value is the fallback. Runtime vars written via `setVariable` land in the runner's `variables` map, so `${name}` / `{{name}}` interpolation in later steps resolves them (e.g. login → store accessToken → set-auth-bearer `${accessToken}`). Editor `ApiStepDetail` set-variable form exposes both Literal Value and Extract Path fields.
- **Feature bundle authSetup:** `feature-bundle.ts` adds `authSetup` to any feature with token-protected endpoints — a ready login→extract→set-auth-bearer recipe (seed creds + login body) plus the `save-to-profile` api-config mapping to persist the token so `profile.api` auto-injects Authorization across a suite. Surfaced to the agent via `get_feature_bundle` + `agent/prompt.ts` workflow.
